...
class EmailPreviewContent(Component):
    ...
    def __init__(self, parent: Misc):
        ...
        body_scrollbar.pack(side="right", fill="y", padx=(0, 5), pady=10)
        self.body_text.configure(yscrollcommand=body_scrollbar.set)

        self.body_text.pack(side="left", fill="both", expand=True, padx=(10, 0), pady=10)
        ...
        self.attachments_frame = Component(self, show_label=False)
        self.attachments_frame.pack(fill="both", expand=False)
        self.attachments_frame.grid_columnconfigure(0, weight=1)

        self.attachments_tree = ttk.Treeview(self.attachments_frame, selectmode="browse", show="tree", height=2)
        ...
        def on_attachments_show_var_write():
            if self.attachments_show_var.get():
                self.attachments_tree.grid()
            else:
                self.attachments_tree.grid_remove()

        self.attachments_show_var.trace_add("write", lambda name, index, mode: on_attachments_show_var_write())

        self.attachments_show_checkbutton.grid(row=0, sticky="nsew", padx=10, pady=10)
        self.attachments_tree.grid_configure(row=1, sticky="nsew", padx=10, pady=(0, 10))
        self.attachments_tree.grid_remove()
    ...
    def _on_preview_button_click(self, e: eb.Event):
        ...
        # notify
        eb.bus.publish("email_preview_content.preview_frame#change")

        return True

    def _udpate_content(self):
