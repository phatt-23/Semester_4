...
class ComposeView(Component):
    def __init__(self, parent: Misc):
        ...
        self.email_editor.grid_configure(row=0, sticky="nsew", padx=10)

        self.open_in_new_window_button = ttk.Button(
            self.left_pane, 
            text="Open in new window", 
            image=ImageManager().get("icon_open_in_new"), 
            compound="left", style="LeftAlign.TButton",
            command=lambda: eb.bus.publish(EventNames.OPEN_IN_NEW_WINDOW)
        )
        self.open_in_new_window_button.grid(row=1, sticky="e", padx=10, pady=10)

        self.attach_sidebar = AttachmentSidebar(self)

        self.paned_window.add(self.left_pane, weight=5)
        self.paned_window.add(self.attach_sidebar, weight=3)
